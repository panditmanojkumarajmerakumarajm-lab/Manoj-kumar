...
import { collection, query, where, orderBy, onSnapshot, doc, updateDoc, increment, getDoc, setDoc } from 'firebase/firestore';
import { db } from '../firebase';
import { Transaction } from '../types';
import { formatINR } from '../lib/utils';
import { toast } from 'react-hot-toast';
import { Check, X, ShieldAlert, Percent, Save } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="glass-card p-8 rounded-3xl">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <h3 className="text-xl font-bold">Pricing Settings (Pushpendra Control)</h3>
        </div>
        <div className="flex flex-col md:flex-row items-end gap-6">
          <div className="flex-1 w-full">
            <label className="block text-sm font-medium text-slate-400 mb-2">Profit Margin Percent (%)</label>
          ...
          <button
            ...
            className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-xl font-bold transition-all shadow-lg shadow-blue-900/40 flex items-center justify-center space-x-2"
          >
            <Save size={18} />
            <span>{savingMargin ? 'Saving...' : 'Save Price'}</span>
          </button>
        </div>
        <p className="text-[10px] text-slate-500 mt-4 uppercase tracking-widest font-medium">
          Note: This will add the specified percentage to all API services.
        </p>
      </div>
